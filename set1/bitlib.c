...
char *bin2HexString(unsigned char *binary, int len) {
	...
	unsigned char leading_byte = binary[0];
	...
	hex_str[j--] = bin2sym16(binary[0] & BOT_FOUR);
	// would == 0 or >= 0 be a faster op????
	if (j >= 0) hex_str[j] = bin2sym16((binary[0] & TOP_FOUR) >> 4);
...
unsigned char *hex2bin(char *hex, int *bin_len) {
	...
	int j = new_len -1; //least sig position in the binary array
...
static char *bin2six(unsigned char *binary, int len) {
